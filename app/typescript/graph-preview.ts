...
function initializeSliders(settings: ForceSettings): void {
    // Set slider values
    const centerSlider = document.getElementById(
        "center-force"
    ) as HTMLInputElement;
    const chargeSlider = document.getElementById(
        "charge-strength"
    ) as HTMLInputElement;
    const collisionSlider = document.getElementById(
        "collision-force"
    ) as HTMLInputElement;
    const linkStrengthSlider = document.getElementById(
        "link-strength"
    ) as HTMLInputElement;
    const linkDistanceSlider = document.getElementById(
        "link-distance"
    ) as HTMLInputElement;
    const nodeSizeSlider = document.getElementById(
        "node-size"
    ) as HTMLInputElement;
    ...
    if (chargeSlider) {
        ...
        document.getElementById("charge-strength-value")!.textContent =
            settings.chargeStrength.toString();
    }
    if (collisionSlider) {
        collisionSlider.value = settings.collisionForce.toString();
        document.getElementById("collision-force-value")!.textContent =
            settings.collisionForce.toFixed(1);
    }
    if (linkStrengthSlider) {
        linkStrengthSlider.value = settings.linkStrength.toString();
        document.getElementById("link-strength-value")!.textContent =
    ...
    if (linkDistanceSlider) {
        ...
        document.getElementById("link-distance-value")!.textContent =
            settings.linkDistance.toString();
    }
    if (nodeSizeSlider) {
...
export function createGraph(config: GraphConfig): void {
    ...
    Promise.all([d3.csv(config.nodesUrl!), d3.csv(config.edgesUrl!)])
        .then(([rawNodes, rawEdges]) => {
            ...
            nodes = rawNodes.map((d) => {
                ...
                if (
                    ...
                    nodeColor =
                        config.colorMap[d[config.colorField]] || nodeColor;
                }
                ...
                const hasFollowerData =
                    d.playlist_followers &&
                    d.playlist_followers !== "" &&
                    +d.playlist_followers > 0;
                ...
                return {
                    ...
                    playlist_followers: d.playlist_followers
                        ? +d.playlist_followers
                        : 0,
                    x: Math.random() * width,
            ...
            let maxFollowers = Math.max(
                ...nodes.map((n) => n.playlist_followers || 0),
                1
            );
            ...
            links.forEach((link) => {
                const sourceId =
                    typeof link.source === "string"
                        ? link.source
                        : link.source.playlist_id;
                const targetId =
                    typeof link.target === "string"
                        ? link.target
                        : link.target.playlist_id;
            ...
            let maxOutgoing = Math.max(
                ...nodes.map((n) => outgoingCount[n.playlist_id] || 0),
                1
            );
            let maxIncoming = Math.max(
                ...nodes.map((n) => incomingCount[n.playlist_id] || 0),
                1
            );
                    ...
                    a.findIndex((t) => {
                        const vSource =
                            typeof v.source === "string"
                                ? v.source
                                : v.source.playlist_id;
                        const vTarget =
                            typeof v.target === "string"
                                ? v.target
                                : v.target.playlist_id;
                        const tSource =
                            typeof t.source === "string"
                                ? t.source
                                : t.source.playlist_id;
                        const tTarget =
                            typeof t.target === "string"
                                ? t.target
                                : t.target.playlist_id;
            ...
            function nodeSize(d: NodeData): number {
                ...
                if (basis === "incoming") {
                    scalingValue =
                        (incomingCount[d.playlist_id] || 0) / maxIncoming;
                } else if (basis === "followers") {
                    // Use follower count, with a minimum of 1 to avoid very small nodes
                    scalingValue = Math.sqrt(
                        (d.playlist_followers || 0) / maxFollowers
                    );
                } else {
                    // outgoing (default)
                    scalingValue =
                        (outgoingCount[d.playlist_id] || 0) / maxOutgoing;
                }
            ...
            const collisionForce = d3
                ...
                .radius((d: NodeData) => nodeSize(d) + 3)
                .iterations(3)
            ...
            function ticked(): void {
                ...
                node.attr("cx", (d: NodeData) => d.x!).attr(
                    "cy",
                    (d: NodeData) => d.y!
                );
            }
            ...
            function highlightNeighbors(
                ...
                const neighbors = links
                    ...
                    .map((link) => {
                        const source = link.source as NodeData;
                        const target = link.target as NodeData;
                        return source.playlist_id === d.playlist_id
                            ? target
                            : source;
                    });
                ...
                d3.selectAll("line")
                    .filter(
                        (linkData) =>
                            !neighborsLinks.includes(
                                linkData as ProcessedEdgeData
                            )
                    )
                    .transition()
                    .duration(300)
                ...
                d3.selectAll("line")
                    .filter((linkData) =>
                        neighborsLinks.includes(linkData as ProcessedEdgeData)
                    )
                    .transition()
                    .duration(300)
                    .ease(d3.easeCubicInOut)
                    .attr("opacity", (linkData) =>
                        linkOpacity(linkData as ProcessedEdgeData)
                    );

                d3.selectAll("circle")
                    .filter((nodeData) =>
                        neighbors.includes(nodeData as NodeData)
                    )
                    .attr("opacity", 1);
            ...
            const chargeStrengthSlider = document.getElementById(
                "charge-strength"
            ) as HTMLInputElement;
            const centerForceSlider = document.getElementById(
                "center-force"
            ) as HTMLInputElement;
            const collisionForceSlider = document.getElementById(
                "collision-force"
            ) as HTMLInputElement;
            const nodeSizeSlider = document.getElementById(
                "node-size"
            ...
            if (lineWidthSlider) {
                lineWidthSlider.addEventListener("input", function () {
                    ...
                    // Update line widths
                    link.transition().duration(300).attr("stroke-width", value);
                });
            }
